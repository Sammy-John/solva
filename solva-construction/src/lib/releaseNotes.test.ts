...
describe('getLatestChanges', () => {
  it('returns release items for 1.2.0', () => {
    const result = getLatestChanges('1.2.0');
    expect(result.version).toBe('1.2.0');
    expect(result.items.join(' ')).toContain('Construction Starter Schedule');
    expect(result.items.join(' ')).toContain('Settings');
  });
